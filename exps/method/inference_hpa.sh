...
MODEL_VERSION=original         # original, autoreg
PRE_NORM=0
OA=0
...
# Decoder. No need in the final version
DECODE_ON=0
NUM_POS=1
NUM_QUERIES=1
COEF_DECODE=0.1
...
# Others.
OUTPUT_DIR=${CURDIR}/checkpoints/inference

CHECKPOINT=$1

echo 'Begin evaluation.'

CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --master_port=$((RANDOM + 10000)) --use_env --nproc_per_node=${NUM_GUPS} ${CURDIR}/execute.py --category ${CATEGORY} \
...
--loss_weight_trans_l2 ${COEF_TRANS_L2} --loss_weight_rot_l2 ${COEF_ROT_L2} --loss_weight_rot_cd ${COEF_ROT_CD} \
...
--eval-epochs 0 --eval-only 1 --checkpoint ${CHECKPOINT} -eb 8
